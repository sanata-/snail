...
func (fh *fileHandler) serveFile(w http.ResponseWriter, r *http.Request, name string) (int, error) {
	...
	for _, hiddenPath := range fh.hide {
		// Case-insensitive file systems may have loaded "SnailShell" when
		// we think we got "SnailShell", which poses a security risk if we
		// aren't careful here: case-insensitive comparison is required!
		// In other words, trying to serve another file with the same name
		// as the active config file will result in a 404 when it shouldn't.
		if strings.EqualFold(d.Name(), path.Base(hiddenPath)) {
			return http.StatusNotFound, nil
		}
	}

	// Errors generated by ServeContent are written immediately
